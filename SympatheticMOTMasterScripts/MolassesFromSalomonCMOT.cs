...
public class Patterns : MOTMasterScript
{
    ...
    public Patterns()
    {
        ...
        Parameters["PumpingPower"] = 2.2;//this is the beam power used to optically pump before imaging
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"], 0 ,1,"D2EOMenable");

        //flash probe on for 100us prior to imaging to redistribute the atoms
        //p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"], 0, 1, "aom1enable");
  
        //Imaging
        p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"]+ (int)Parameters["ImageDelay"], 0, 100, "aom1enable");
        p.DownPulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"] + (int)Parameters["ImageDelay"], 0, 100, "CameraTrigger"); //take an image of the cloud after D1 stage
