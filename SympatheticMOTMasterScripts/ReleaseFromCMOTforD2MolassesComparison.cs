...
// This script is for running grey molasses after a CMOT, during which the coil currents are ramped up. 
...
public class Patterns : MOTMasterScript
{
    ...
    public Patterns()
    {
        ...
        Parameters["MotRepumpAmplitude"] = 3.4;// repump amplitude, as voltage applied to VCA
        Parameters["XCoilCurrent"] = 0.9;
        Parameters["YCoilCurrent"] = 6.55;
        Parameters["ZCoilCurrent"] = 1.37;

        //CMOT settings
        Parameters["CMOTFinalTopCurrent"] = 1.9;
        Parameters["CMOTFinalBottomCurrent"] = 1.9;
        Parameters["CMOTRampTime"] = 100;

        //Molasses Settings
        Parameters["MolassesStartTime"] = 70000;
        Parameters["MolassesPrincipalFrequency"]=200.0;
        Parameters["MolassesPrincipalAmplitude"] = 6.0;
        Parameters["MolassesRepumpDetuning"] = 796.0;
        Parameters["MolassesPulseLength"]= 30;
        Parameters["offsetlockvcofrequency"] = 5.0;//-300MHz/V
        Parameters["offsetlockHCval"] =5.0;//Should be same as on hardware controller, to prevent unlocking when remote control ends
        Parameters["MolassesRepumpAmplitude"] = 4.0;
        Parameters["DoMolassesPulse"] = false;
        Parameters["FieldSwitchOffTime"] = 0;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        //Imaging
        p.Pulse((int)Parameters["MolassesStartTime"] + (int)Parameters["ImageDelay"], -2, 1, "aom3enable");
        p.Pulse((int)Parameters["MolassesStartTime"] +(int)Parameters["ImageDelay"], -1, 100, "aom1enable");
        p.Pulse((int)Parameters["MolassesStartTime"] +(int)Parameters["ImageDelay"], -2, 100, "D2EOMenable");
        p.DownPulse((int)Parameters["MolassesStartTime"] +(int)Parameters["ImageDelay"], 0, 100, "CameraTrigger"); //take an image of the cloud after D1 stage
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddAnalogValue("offsetlockfrequency", 0, (double)Parameters["offsetlockHCval"]);
        p.AddLinearRamp("offsetlockfrequency", 1,10000 , (double)Parameters["offsetlockvcofrequency"]);//setting up molasses parameters
        

        //CMOT
        //p.AddLinearRamp("aom3amplitude", (int)Parameters["MOTEndTime"] - 10, 10, 3.0);
        //p.AddLinearRamp("aom3frequency", (int)Parameters["MOTEndTime"] - 10, 10, 180.0);
        p.AddLinearRamp("TopTrappingCoilcurrent", (int)Parameters["MOTEndTime"] - (int)Parameters["CMOTRampTime"], (int)Parameters["CMOTRampTime"], (double)Parameters["CMOTFinalTopCurrent"]);
        p.AddLinearRamp("BottomTrappingCoilcurrent", (int)Parameters["MOTEndTime"] - (int)Parameters["CMOTRampTime"], (int)Parameters["CMOTRampTime"], (double)Parameters["CMOTFinalBottomCurrent"]);
        ...
        //Taking the pictures
        p.AddAnalogValue("D2EOMfrequency", (int)Parameters["MolassesStartTime"]  +(int)Parameters["ImageDelay"] - 3, (double)Parameters["absImageRepumpDetuning"]);
        p.AddAnalogValue("D2EOMamplitude", (int)Parameters["MolassesStartTime"] +(int)Parameters["ImageDelay"] - 3, (double)Parameters["absImageRepumpAmplitude"]);
        p.AddAnalogValue("aom1frequency", (int)Parameters["MolassesStartTime"] +  (int)Parameters["ImageDelay"] - 1, (double)Parameters["absImageDetuning"]);
        p.AddAnalogValue("aom1amplitude", (int)Parameters["MolassesStartTime"] + (int)Parameters["ImageDelay"] - 1, (double)Parameters["absImagePower"]);
        p.AddAnalogValue("aom1amplitude", (int)Parameters["Frame1Trigger"] - 1, (double)Parameters["backgroundImagePower"]);

        //ramp offsetlock back to start value to prevent unlocking when remote control ends
        p.AddLinearRamp("offsetlockfrequency", (int)Parameters["PatternLength"]-10001,10000 , (double)Parameters["offsetlockHCval"]);
