...
// This script is for comparing the MOT to D2 molasses, for capture efficiency etc. 
...
public class Patterns : MOTMasterScript
{
    ...
    public Patterns()
    {
        ...
        Parameters["offsetlockHCval"]= 0.0;
        Parameters["MolassesRepumpAmplitude"] = 3.4;
        ...
        Parameters["backgroundImagePower"] = 1.0;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        //Imaging
        p.Pulse((int)Parameters["MOTEndTime"]+(int)Parameters["ImageDelay"], -2, 1, "aom3enable");
        p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["ImageDelay"], -1, 100, "aom1enable");
        p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["ImageDelay"], -2, 100, "D2EOMenable");
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddAnalogValue("offsetlockfrequency", 0, (double)Parameters["offsetlockHCval"]);//setting up molasses parameters
        p.AddLinearRamp("offsetlockfrequency", 1,10000, (double)Parameters["offsetlockvcofrequency"]);
        ...
        p.AddAnalogValue("aom1frequency", (int)Parameters["MOTEndTime"] + (int)Parameters["ImageDelay"] + (int)Parameters["ImageDelay"] - 1, (double)Parameters["absImageDetuning"]);
        p.AddAnalogValue("aom1amplitude", (int)Parameters["MOTEndTime"] + (int)Parameters["ImageDelay"] + (int)Parameters["ImageDelay"] - 1, (double)Parameters["absImagePower"]);
        p.AddAnalogValue("aom1amplitude", (int)Parameters["Frame1Trigger"] - 1, (double)Parameters["backgroundImagePower"]);

        p.AddLinearRamp("offsetlockfrequency", (int)Parameters["PatternLength"] - 10001, 10000, (double)Parameters["offsetlockHCval"]);//return offset lock to hc value
