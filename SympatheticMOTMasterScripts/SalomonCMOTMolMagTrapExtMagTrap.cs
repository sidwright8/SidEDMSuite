...
public class Patterns : MOTMasterScript
{
    ...
    public Patterns()
    {
        ...
        Parameters["MagTrapHoldTime"] = 500;// time in internal magnetic trap before transfer
        Parameters["MagTrapDelay"] = 3;
        Parameters["TopExtTrapCurrent"] =3.0;
        Parameters["BottomExtTrapCurrent"] = 3.15;
        Parameters["MagTrapTransferTime"] = 500; //time over which ramping occurs
        Parameters["ExtMagTrapHoldTime"] = 5000; // hold time in external magnetic trap
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        //Imaging
        p.Pulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"] + (int)Parameters["MagTrapDelay"] + (int)Parameters["MagTrapRampTime"] + (int)Parameters["MagTrapHoldTime"] + (int)Parameters["MagTrapTransferTime"] + (int)Parameters["ExtMagTrapHoldTime"] + (int)Parameters["MagTrapTransferTime"] + (int)Parameters["ImageDelay"], 0, 100, "aom1enable");
        p.DownPulse((int)Parameters["MOTEndTime"] + (int)Parameters["MolStartDelay"] + (int)Parameters["MolassesPulseLength"] + (int)Parameters["MagTrapDelay"] + (int)Parameters["MagTrapRampTime"] + (int)Parameters["MagTrapHoldTime"] + (int)Parameters["MagTrapTransferTime"] + (int)Parameters["ExtMagTrapHoldTime"] + (int)Parameters["MagTrapTransferTime"] + (int)Parameters["ImageDelay"], 0, 100, "CameraTrigger"); //take an image of the cloud after D1 stage
