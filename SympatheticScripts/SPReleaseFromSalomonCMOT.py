...
path = "C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\ReleaseFromSalomonCMOT.cs" 
...
def ScanCMOTTime(HoldTimes, repeats):
	dic = Dictionary[String, Object]()
	if not repeats:
		repeats = 1
	mm.SetScriptPath(path)
	for i in range(repeats):
		...
		for j in HoldTimes:
			dic["CMOTTime"] = j
			print('CMOTTime Hold Time: ' +str(j))
			mm.Run(dic)	
...
def ScanCMOTFinalDetuning(vals, repeats):
	dic = Dictionary[String, Object]()
	if not repeats:
		repeats = 1
	mm.SetScriptPath(path)
	for i in range(repeats):
		print('repeat number: '+str(i) )
		for j in vals:
			dic["CMOTFinalDetuning"] = j
			print('CMOT Final Detuning: ' +str(j))
			mm.Run(dic)	
	
	return 0

def ScanMOTRedistPower(vals, repeats):
	dic = Dictionary[String, Object]()
	if not repeats:
		repeats = 1
	mm.SetScriptPath(path)
	for i in range(repeats):
		print('repeat number: '+str(i) )
		for j in vals:
			dic["MOTRedistPower"] = j
			print('MOT Redistribution Power: ' +str(j))
			mm.Run(dic)	
