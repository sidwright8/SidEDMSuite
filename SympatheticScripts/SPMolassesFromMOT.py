...
def ScanXCoilCurrentAndMeasureExpansion(xcoilvalues,ExpansionTimes):
	...
	dic = Dictionary[String, Object]()
	while(count < endcount):
		dic["MolXCoilCurrent"] = xcoilvalues[count]
		print(xcoilvalues[count])
		i=0
		while(i<len(ExpansionTimes)):
			dic["ImageDelay"]=ExpansionTimes[i]
			mm.Run(dic)
			print(ExpansionTimes[i])
			i = i+1
		count = count+1
	return 0
		
def ScanYCoilCurrentAndMeasureExpansion(ycoilvalues,ExpansionTimes):
	count = 0
	list = ycoilvalues
	endcount = len(ycoilvalues)
	mm.SetScriptPath(path)
	
	dic = Dictionary[String, Object]()
	while(count < endcount):
		dic["MolYShimCoilCurrent"] = ycoilvalues[count]
		print(ycoilvalues[count])
		i=0
		while(i<len(ExpansionTimes)):
			dic["ImageDelay"]=ExpansionTimes[i]
			print(ExpansionTimes[i])
...
def ScanZCoilCurrentAndMeasureExpansion(zcoilvalues,ExpansionTimes):
	count = 0
	list = zcoilvalues
	endcount = len(zcoilvalues)
	mm.SetScriptPath(path)
	
	dic = Dictionary[String, Object]()
	while(count < endcount):
		dic["MolZShimCoilCurrent"] = zcoilvalues[count]
		print(zcoilvalues[count])
		i=0
		while(i<len(ExpansionTimes)):
			dic["ImageDelay"]=ExpansionTimes[i]
			print(ExpansionTimes[i])
...
def ScanMRAAndMeasureExpansion(MRAvalues,ExpansionTimes):
	count = 0
	list = MRAvalues
	endcount = len(list)
	dic = Dictionary[String, Object]()
