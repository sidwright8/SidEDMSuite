...
def ScanImageTrigger(values):
	count = 0
	list = values
	endcount = len(list)
	dic = Dictionary[String, Object]()
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\BallisticExpansion.cs")
...
def RepeatScansIT(values, numberofrepeats):
	j = 0
	while(j < numberofrepeats):
		ScanImageTrigger(values)
		j = j+1
	return 0
