...
def ScanReleaseTimeAI(initial,final,interval):
	count = 0
	endcount = (final-initial)/interval
	dic = Dictionary[String,Object]()
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\ReleaseRecaptureAbsImage.cs")
	while(count < endcount+1):
		dic["ReleaseTime"] = (count*interval)+initial
		dic["Frame1Trigger"] = 100200 + (count*interval)+initial
		mm.Run(dic)
		count = count + 1
	return 0
