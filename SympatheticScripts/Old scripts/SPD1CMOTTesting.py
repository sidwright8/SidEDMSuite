...
def ScanRepumperPower(values):
	count = 0
	list = values
	endcount = len(list)
	dic = Dictionary[String, Object]()
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\D1 TestingCMOT.cs")
	while(count < endcount):
		dic["D1RepumperPower"] = list[count]
		mm.Run(dic)
		count=count + 1
	return 0

def ScanPictureTime(values):
	count = 0
	list = values
	endcount = len(list)
	dic = Dictionary[String, Object]()
...
def RepeatScansPT(values, numberofrepeats):
	j = 0
	while(j < numberofrepeats):
		ScanPictureTime(values)
		j = j+1
	return 0

def ScanPrincipalDetuning(values):
	count = 0
	list = values
	endcount = len(list)
	dic = Dictionary[String, Object]()
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\D1 TestingCMOT.cs")
	while(count < endcount):
		dic["D1PrincipalDetuning"] = list[count]
...
def RepeatScansOF(values, numberofrepeats):
	j = 0
	while(j < numberofrepeats):
		ScanRepumperDetuning(values)
		j = j+1
	return 0
...
def ScanD1Duration(values):
	...
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\D1 TestingCMOT.cs")
	while(count < endcount):
		dic["D1Duration"] = list[count]
		mm.Run(dic)
		count=count + 1
	return 0

def RepeatScansD1Duration(values, numberofrepeats):
	j = 0
	while(j < numberofrepeats):
		ScanD1Duration(values)
		j = j+1
	return 0
