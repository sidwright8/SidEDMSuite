...
def ScanMagLoad(initial,final,interval):
	...
	mm.SetScriptPath("C:\\ExperimentControl\\EDMSuite\\SympatheticMOTMasterScripts\\LoadMagTrap.cs")
...
def ScanMagLoadRand(values):
	...
	while(count < endcount):
		dic["MagTrapDuration"] = list[count]
		dic["Frame1Trigger"] = 100000 + list[count]
		mm.Run(dic)
		count = count + 1
	return 0

def RepeatScansMLRand(values,numberofrepeats):
	j = 0
	while(j < numberofrepeats):
		ScanMagLoadRand(values)
		j = j+1
	return 0
